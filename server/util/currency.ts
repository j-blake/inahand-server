import { IsoCurrency } from '../@types/IsoCurrency';
...
const getCurrencyList = (): Promise<IsoCurrency[]> =>
  Promise.resolve(iso4217.ISO_4217.CcyTbl.CcyNtry);

export const fetchIsoCurrency = async (
  code: string
): Promise<IsoCurrency | null> => {
  const currencyList = await getCurrencyList();
  return currencyList.find((currency) => code === currency.Ccy) ?? null;
};
...
export const convertToMinorUnits = (
  ...
  Math.round(amount * Math.pow(10, parseInt(currency?.CcyMnrUnts || '0') || 0));

export const convertToMajorUnits = (
  amount: number,
  currency: IsoCurrency
): number => amount / Math.pow(10, parseInt(currency?.CcyMnrUnts || '0') || 0);
